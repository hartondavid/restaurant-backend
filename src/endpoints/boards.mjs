...
router.post('/addBoard', userAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        const result = await (await databaseManager.getKnex())('boards').insert({ number, waiter_id: userId }).returning('id');
...
router.post('/addProductToBoard', userAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (board) {
            const result = await (await databaseManager.getKnex())('orders').insert({ board_id: board_id, waiter_id: userId }).returning('id');
            // Handle different database return formats
            const id = Array.isArray(result) ? result[0].id : result.id;
